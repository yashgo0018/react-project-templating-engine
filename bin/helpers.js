...
function deleteIfUnselectedModuleFile(
  filePath,
  supportedModules,
  selectedModules
) {
...
function deleteUnselectedModuleCode(
  filePath,
  supportedModules,
  selectedModules
) {
  ...
  for (let i = 0; i < codeLines.length; i++) {
    ...
    (() => {
      if (state == 0) {
        ...
        state = requiredModules.some((moduleName) =>
          selectedModules.includes(moduleName)
        )
          ? 1
          : 2;
...
function handleDirectory(dirPath, supportedModules, selectedModules) {
  ...
  if (
    !requiredModules.some((moduleName) => selectedModules.includes(moduleName))
  ) {
